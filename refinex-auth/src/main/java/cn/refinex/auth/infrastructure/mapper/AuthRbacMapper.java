...
public interface AuthRbacMapper {
    ...
    @Select({
            ...
            "WHERE ru.user_id = #{userId}",
            "  AND ru.deleted = 0 AND r.deleted = 0 AND rmo.deleted = 0 AND mo.deleted = 0",
            "  AND ru.status = 1 AND r.status = 1 AND mo.status = 1",
            "  AND (ru.estab_id = #{estabId} OR ru.estab_id = 0)",
            "  AND mo.permission_key IS NOT NULL AND mo.permission_key &lt;&gt; ''",
            "<if test='systemId != null'>",
            "  AND mo.system_id = #{systemId}",
            "</if>",
            "UNION",
            ...
            "WHERE ru.user_id = #{userId}",
            "  AND ru.deleted = 0 AND r.deleted = 0 AND rdi.deleted = 0 AND di.deleted = 0",
            "  AND ru.status = 1 AND r.status = 1 AND di.status = 1",
            "  AND (ru.estab_id = #{estabId} OR ru.estab_id = 0)",
            "  AND di.permission_key IS NOT NULL AND di.permission_key &lt;&gt; ''",
            "<if test='systemId != null'>",
            "  AND di.system_id = #{systemId}",
            "</if>",
            "</script>"
    })
