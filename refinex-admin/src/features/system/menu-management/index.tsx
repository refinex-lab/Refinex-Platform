...
const menuFormSchema = z.object({
  ...
  component: z.string().trim().max(255, '组件路径最长 255 位').optional(),
...
const menuOpFormSchema = z.object({
  ...
  pathPattern: z.string().trim().max(255, '路径模式最长 255 位').optional(),
  permissionKey: z.string().trim().max(128, '权限标识最长 128 位').optional(),
  status: z.enum(['1', '2']),
  sort: z.string().trim().max(6, '排序值过大').optional(),
})
...
export function MenuManagementPage() {
  ...
  async function loadMenuDetail(menuId: number) {
    ...
    try {
      ...
      menuDetailForm.reset({
        ...
        menuType: String(detail.menuType ?? 1) as '0' | '1' | '2',
        path: detail.path ?? '',
  ...
  async function submitCreateMenu(values: MenuFormValues) {
    ...
    try {
      const payload: MenuCreateRequest = {
        ...
        component: toOptionalString(values.component),
  ...
  function renderTreeNode(node: MenuTreeNodeUI, level = 0) {
    ...
    return (
      <div key={node.id ?? `${node.menuCode}-${level}`}>
        ...
        >
          {hasChildren ? (
            <Button type='button' variant='ghost' size='icon' className='h-6 w-6' onClick={() => toggleNode(nodeId)}>
  ...
  return (
    <>
        ...
        <div className='grid h-full gap-2 xl:grid-cols-[340px_1fr]'>
          <Card className='py-3 gap-3'>
            ...
            <CardContent className='space-y-3'>
              <Select value={systemId ? String(systemId) : ''} onValueChange={(value) => setSystemId(Number(value))}>
              ...
              <div className='flex items-center gap-2'>
                ...
                />
                <Button type='button' variant='outline' size='icon' onClick={() => systemId && loadMenuTree(systemId)}>
          ...
          <Card className='py-3 gap-3'>
            ...
            <CardContent className='space-y-4'>
              {!selectedMenuId ? (
                <div className='py-16 text-center text-muted-foreground'>请先在左侧选择一个菜单节点</div>
              ) : loadingMenuDetail ? (
                ...
                <Tabs defaultValue='basic' className='space-y-3'>
                  ...
                  <TabsContent value='basic'>
                    <Form {...menuDetailForm}>
                      <form onSubmit={menuDetailForm.handleSubmit(submitUpdateMenu)} className='space-y-4'>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input {...field} placeholder='例如：features/system/menu' />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input {...field} placeholder='例如：menu:manage:view' />
                                </FormControl>
                                <FormMessage />
                              </FormItem>
                            )}
                        ...
                        <div className='grid gap-4 md:grid-cols-3'>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel>缓存</FormLabel>
                                <Select value={field.value} onValueChange={field.onChange}>
                                  <FormControl>
                                    <SelectTrigger>
                                      <SelectValue placeholder='请选择缓存状态' />
                                    </SelectTrigger>
                                  </FormControl>
                                  <SelectContent>
                                    <SelectItem value='0'>否</SelectItem>
                        ...
                        <div className='flex items-center justify-end gap-2'>
                          ...
                          <Button type='submit' disabled={savingMenuDetail}>
                            {savingMenuDetail ? <Loader2 className='mr-2 h-4 w-4 animate-spin' /> : null}
                            保存菜单
                          </Button>
                        </div>
                      </form>
      ...
      <Dialog open={menuDialogOpen} onOpenChange={(open) => (open ? setMenuDialogOpen(true) : closeCreateMenuDialog())}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>组件路径</FormLabel>
                      <FormControl>
                        <Input {...field} placeholder='例如：features/system/menu-management' />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select value={field.value} onValueChange={field.onChange}>
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder='请选择缓存策略' />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value='0'>否</SelectItem>
      ...
      <Dialog open={menuOpDialogOpen} onOpenChange={(open) => (open ? setMenuOpDialogOpen(true) : closeMenuOpDialog())}>
        <DialogContent className='sm:max-w-3xl'>
          <DialogHeader>
            <DialogTitle>{editingMenuOp?.id ? '编辑菜单操作' : '新建菜单操作'}</DialogTitle>
            <DialogDescription>操作编码由后端自动生成，建议重点维护权限标识和路径。</DialogDescription>
          </DialogHeader>
          <Form {...menuOpForm}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} placeholder='例如：/menus/**' />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>权限标识</FormLabel>
                      <FormControl>
                        <Input {...field} placeholder='例如：menu:manage:edit' />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
