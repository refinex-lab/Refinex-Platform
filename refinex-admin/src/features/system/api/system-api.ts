...
export async function listSystems(query?: SystemListQuery): Promise<SystemDefinition[]> {
  const response = await http.get<SystemDefinition[]>(buildSystemPath('/systems'), {
    params: query,
  })
