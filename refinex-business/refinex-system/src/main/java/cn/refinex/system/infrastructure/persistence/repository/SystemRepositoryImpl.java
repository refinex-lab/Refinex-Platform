...
public class SystemRepositoryImpl implements SystemRepository {
    ...
    public PageResponse<SystemEntity> listSystems(Integer status, String keyword, int currentPage, int pageSize) {
        ...
        for (ScrSystemDo row : rowsPage.getRecords()) {
    ...
    @Override
    public PageResponse<RoleEntity> listRoles(Long estabId, Integer status, String keyword,
        ...
        for (ScrRoleDo row : rowsPage.getRecords()) {
    ...
    @Override
    public long countRoleCode(Long estabId, String roleCode, Long excludeRoleId) {
        LambdaQueryWrapper<ScrRoleDo> query = Wrappers.lambdaQuery(ScrRoleDo.class)
    ...
    @Override
    public List<MenuEntity> listMenus(Long estabId, Long systemId) {
        List<ScrMenuDo> rows = scrMenuMapper.selectList(
                Wrappers.lambdaQuery(ScrMenuDo.class)
                        .eq(ScrMenuDo::getEstabId, estabId)
    ...
    @Override
    public long countMenuCode(Long estabId, Long systemId, String menuCode, Long excludeMenuId) {
    ...
    public MenuEntity insertMenu(MenuEntity menu) {
        ...
        row.setEstabId(menu.getEstabId());
        ...
        row.setIsBuiltin(menu.getIsBuiltin());
    ...
    public PageResponse<MenuOpEntity> listMenuOpsByMenuId(Long menuId, int currentPage, int pageSize) {
        ...
        for (ScrMenuOpDo row : rowsPage.getRecords()) {
    ...
    @Override
    public long countMenusByIdsAndEstabId(Long estabId, List<Long> menuIds) {
        if (menuIds == null || menuIds.isEmpty()) {
            return 0;
        }
        Long count = scrMenuMapper.selectCount(
    ...
    @Override
    public long countMenuOpsByIdsAndEstabId(Long estabId, List<Long> menuOpIds) {
        if (menuOpIds == null || menuOpIds.isEmpty()) {
            return 0;
        }
        return scrMenuOpMapper.countByIdsAndEstabId(estabId, menuOpIds);
    }

    @Override
    public long countDrsInterfacesByIdsAndEstabId(Long estabId, List<Long> drsInterfaceIds) {
        if (drsInterfaceIds == null || drsInterfaceIds.isEmpty()) {
            return 0;
        }
